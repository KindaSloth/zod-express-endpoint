...
export class TypeSafeEndpoint {
    ...
    private emitZodErrorsBoolean(): boolean {
        ...
        if (this.zodErrorMode === 'development' && process.env.NODE_ENV === 'development') return true;

        if (this.zodErrorMode === 'production' && process.env.NODE_ENV === 'production') return true;

        return false;
    }

    create<
        ResponseSchema extends AnyZodObject,
        Params extends AnyZodObject = never,
        Body extends AnyZodObject = never,
        Query extends AnyZodObject = never,
    >(
        schemas: {
            paramsSchema?: Params;
            bodySchema?: Body;
        ...
        return async (req: Request, res: Response) => {
            try {
                const { paramsSchema, bodySchema, querySchema, responseSchema } = schemas;
                ...
                if (responseValidator.success === false && !this.emitZodErrorsBoolean()) {
                    console.error('Zod Response Error', { error: responseValidator.error })
                }
                ...
                return res.status(400).json({ error: this.customUnexpectedError || 'Unexpected Error' });
            }
        };
    }
}
